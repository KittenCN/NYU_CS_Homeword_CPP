...
public:
	...
	void initialize() // 初始化
	{
		loadimage(&im_enemy, _T("bat.png")); // 导入敌人-蝙蝠-图片
...
public:
	...
	void initialize() // 初始化
	{
		TCHAR filename[80];	
		int i = level % 9 + 1;
		_stprintf(filename, _T("landscape%d.png"),i);	
		loadimage(&im_bk, filename); // 依次导入各张背景图片
		loadimage(&im_star,  _T("star.png")); // 导入星星图片，放在终点处的目标	
		...
		if (lands.size()==0 || level > lastlevel)  
		{
			...
			enemies.clear(); // 敌人也要清空掉
			// 第level关，一共有(level+3)/5 个敌人，也就是第1关没有敌人，后面逐渐增加敌人数目
			int numEnemy = (level+3)/5;
			int idStep = lands.size()/(numEnemy+1);  // 敌人在scene中均匀分布到lands上
...
void updateWithoutInput() // 和输入无关的更新
{
	...
	{
		char txt1[]={"success.mp3"};
 		char* txt_name1=txt1;
		PlayMusicOnce(_T(txt_name1));
